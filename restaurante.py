class Restaurantes:
    ...
    def menu(self):
        ...
        8.- TIPOS DE PLATOS CLASIFICADOS
        9.- Salir
        """
    ...
    def agregarPlato(self):
        codigo = input("Codigo de Plato: \n")
        nombre = input("Nombre del Plato: \n")
        precio = int(input("Precio del Plato: \n"))
        descuento = int(input("Digite el porcentaje del Descuento para el Plato: \n"))
        descripcion = input("Descripcion del Plato: \n")
        tipo = input("Tipo de Plato: \n")
        print(self.guardarPlato(codigo, nombre, precio, descuento, descripcion, tipo))
        agregarOtro = input("Desea agregar otro Plato? y/n \n")
    ...
    def guardarPlato(self, codigo, nombre, precioPlato, descuento, descripcion, tipo):
        ...
        return "El Plato {} fue agregado correctamente".format(nombre)
    ...
    def descripcionesPlato(self, posicion, habilitado):
        if(self.habilitado[posicion] == habilitado):
            print("*****DESCRIPCION DEL PLATO {}*****".format(self.nombre[posicion]))
            print("CODIGO DEL PLATO: {}".format(self.codigo[posicion]))
            print("PRECIO DEL PLATO: {} Bs".format(self.precio[posicion]))
            print("PRECIO CON DESCUENTO: {} Bs".format(self.descuento[posicion]))
            print("DESCRIPCION DEL PLATO: {}".format(self.descripcion[posicion]))
            print("TIPO DEL PLATO: {}".format(self.tipo[posicion]))
    ...
    def editarPlato(self):
        print("*********ACTUALIZAR EL PLATO**********")
        posicion = self.buscarPlato(1)
        # self.descripcionesPlato()
        nuevo_precio = int(input("Digite el nuevo precio del plato {}: \n".format(self.nombre[posicion])))
        nuevo_descuento = int(input("Digite el nuevo porcentaje de descuento para el plato {}: \n".format(self.nombre[posicion])))
        nuevo_descripcion = input("Digite la nueva descripcion para el plato {}: \n".format(self.nombre[posicion]))
        nuevo_tipo = input("Digite el nuevo tipo de plato entre (Desayuno, Almuerzo, Cena y Postre) para el nuevo Plato: \n")
    ...
    def buscarTipoPlato(self, tipo):
        ...
        if encontrado == False:
            print("No se encontraron resultados con el tipo de almuerzo {}".format(tipo))
        pass
